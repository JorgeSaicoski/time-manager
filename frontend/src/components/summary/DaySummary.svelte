<script>
    ...
    import { GetDaySummary, UpdateWorkTimeDuration, UpdateBreakTimeDuration, UpdateWorkTimeProjectDuration } from '../../../wailsjs/go/main/App';
    ...
    let currentWorkTimeProject = null;
    let currentWorkTimeProjectHours = 0;
    let currentWorkTimeProjectMinutes = 0;
    let isoDay;
    let loading = false;
    ...
    onMount(async () => {
        ...
        try {
            ...
            daySummary = summary;
            console.log(daySummary.workTimeProjects[0])
    ...
    function changeWorkTimeProject(id, duration) {
        currentWorkTimeProject = id;
        const totalSeconds = Math.floor(duration / 1000000000);
        currentWorkTimeProjectHours = Math.floor(totalSeconds / 3600);
        currentWorkTimeProjectMinutes = Math.floor((totalSeconds % 3600) / 60);
    }
    ...
    async function saveUpdatedDuration(newHours, newMinutes, model) {
        ...
        try {
            ...
            if (model === "workTime") {
                ...
                await UpdateWorkTimeProjectDuration(currentWorkTimeProject, newDurationSeconds);
            ...
            {#if Array.isArray(daySummary.workTimeProjects) && daySummary.workTimeProjects.length > 0}
                ...
                <ul>
                    {#each daySummary.workTimeProjects as workTimeProject}
                        {#if currentWorkTimeProject === workTimeProject.ID}
                            <li class="w-full bg-hover">
                                <input type="number" bind:value={currentWorkTimeProjectHours} min="1" class="text-black w-[35px]" /> hours
                                :
                                <input type="number" bind:value={currentWorkTimeProjectMinutes} min="1" class="text-black w-[35px]" /> minutes
                                <Button label="Save Duration" onClick={() => saveUpdatedDuration(currentWorkTimeProjectHours, currentWorkTimeProjectMinutes, "workTimeProject")}></Button>
                            </li>
                        {:else}
                            <li>
                                {workTimeProject.Project.Name} - {formatDuration(workTimeProject.Duration)}
                                <Button label="Update Duration" onClick={() => changeWorkTimeProject(workTimeProject.ID, workTimeProject.Duration)}></Button>
                            </li>
                        {/if}
                    {/each}
                </ul>
