...
<div
  class="w-full max-w-2xl p-0 space-y-4 bg-secondary rounded-lg shadow-lg text-white"
>
  {#if loading}
            ...
            {#if currentProject === workTime.ID}
              <li class="w-full bg-hover">
                ...
                <Button
                  label="Save Duration"
                  onClick={() =>
                    saveUpdatedDuration(hours, minutes, "workTime")}
                ></Button>
              </li>
            {:else}
              <li>
                {formatTimeToHourAndMinute(workTime.StartTime)} - {formatDuration(
                  workTime.Duration,
                )}
                <Button
                  label="Update Duration"
                  onClick={() => changeProject(workTime.ID, workTime.Duration)}
                ></Button>
              </li>
            {/if}
          {/each}
      ...
      {#if Array.isArray(daySummary.breaks) && daySummary.breaks.length > 0}
        ...
        <ul>
          {#each daySummary.breaks as breakTime}
            ...
            {:else}
              <li>
                {formatTimeToHourAndMinute(breakTime.StartTime)} - {formatDuration(
                  breakTime.Duration,
                )}
                <Button
                  label="Update Duration"
                  onClick={() =>
                    changeBreakTime(breakTime.ID, breakTime.Duration)}
                ></Button>
              </li>
            {/if}
          {/each}
          ...
          {#each daySummary.workTimeProjects as workTimeProject}
            {#if currentWorkTimeProject === workTimeProject.ID}
              <li class="w-full bg-hover">
                <input
                  type="number"
                  bind:value={currentWorkTimeProjectHours}
                  min="1"
                  class="text-black w-[35px]"
                />
                hours :
                <input
                  type="number"
                  bind:value={currentWorkTimeProjectMinutes}
                  min="1"
                  class="text-black w-[35px]"
                />
                minutes
                <Button
                  ...
                  onClick={() =>
                    saveUpdatedDuration(
                      currentWorkTimeProjectHours,
                      currentWorkTimeProjectMinutes,
                      "workTimeProject",
                    )}
                ></Button>
              </li>
            {:else}
              <li>
                {workTimeProject.Project.Name} - {formatDuration(
                  workTimeProject.Duration,
                )}
                <Button
                  label="Update Duration"
                  onClick={() =>
                    changeWorkTimeProject(
                      workTimeProject.ID,
                      workTimeProject.Duration,
                    )}
                ></Button>
